...
import { fetchPort, removePort, addPort } from '../services/api';
...
export default {
  ...
  effects: {
    *fetchPort({ payload }, { call, put }) {
      const response = yield call(fetchPort, payload);
      if(response.ResponseCode != unauth_code){
        if(response.ResponseState.toLowerCase() != 'success'){
          ...
          const result = {
            list: response.Data.portInfo.data,
            pagination: {
              total: response.Data.portInfo.total,
            },
          };
          yield put({
            type: 'save',
    ...
    *removePort({ payload, callback }, { call, put }) {
      const response = yield call(removePort, payload);
      if(response.ResponseCode != unauth_code){
        if(response.ResponseState.toLowerCase() != 'success'){
          notification['error']({
            message: '操作提示',
