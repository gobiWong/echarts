      ...
      <Form>
         ...
         <FormItem labelCol={{ span: 5 }} wrapperCol={{ span: 15 }} label="用户账号">
          {form.getFieldDecorator('user_account', {
            initialValue: formAddValues.user_account,
            validateFirst: true,
        ...
        <FormItem labelCol={{ span: 5 }} wrapperCol={{ span: 15 }} label="用户密码">
          {form.getFieldDecorator('user_password', {
            initialValue: formAddValues.user_password,
            validateFirst: true,
            // rules: [{ required: true, whitespace:true, message: '请输入' }],
          })(<Input placeholder="请输入" />)}
        </FormItem>
        ...
        <FormItem labelCol={{ span: 5 }} wrapperCol={{ span: 15 }} label="余额地址">
          {form.getFieldDecorator('balance_address', {
            initialValue: formAddValues.balance_address,
            validateFirst: true,
            // rules: [{ required: true, whitespace:true, message: '请输入' }],
          })(<Input placeholder="请输入" />)}
        </FormItem>
...
@connect(({ messagePort, loading }) => ({
  messagePort,
  loading: loading.models.messagePort,
}))
...
export default class PortList extends PureComponent {
  ...
  componentDidMount() {
    ...
    //获取数据
    dispatch({
      type: 'messagePort/fetchMessagePort',
      payload: {},
    });
  ...
  handleDeleteData = (e, text, data) => {
    ...
    dispatch({
      type: 'messagePort/removeMessagePort',
      payload: {
        messagePort_ids: [text.messagePort_id],
        filters: this.state.formQueryValues
      },
      callback: () => {
        this.setState({
  ...
  handleEdit = (e, text, data) => {
    ...
    const formValuesArr = {
      messagePort_id: text.messagePort_id,
      ...
      user_id:text.user_id,
      balance_address:text.balance_address,
    };
  ...
  handleAdd = (fields, form) => {
    ...
    let params = {
      messagePort_id: this.state.formAddValues.messagePort_id,
      user_account:fields.user_account,
      user_password:fields.user_password,
      user_id:fields.user_id,
      balance_address:fields.balance_address,
    ...
    dispatch({
      type: 'messagePort/addMessagePort',
      payload: params,
      callback: (result) => {
